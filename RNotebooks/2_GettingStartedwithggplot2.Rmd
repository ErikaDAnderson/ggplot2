...
mpg %>%
  mutate(cty_metric = round(235.214/cty,2),
         hwy_metric = round(235.214/hwy,2)) %>%
...
# most variation in a model
mpg %>%
  group_by(model) %>%
  count() %>%
  arrange(-n)

# remove redundant specification in model name then count models per manufacturer
mpg %>%
